...
class NLMChemLoader:
    ...
    def __init__(self, nlmchem_filepath, chebi, min_role_char_length=4):
        print("loading NLMChem dataset from:", nlmchem_filepath)
        print("loading Chebi to add roles which can be lexically found in the text snippets...")
        self.nlmchem_filepath = nlmchem_filepath        
    ...
    def __collect_entities(self):
        filelist = []
        text_data = []
        for root, dirs, files in os.walk(os.path.join(self.nlmchem_filepath, "FINAL_v1", "ALL")):
            for file in files:
                f = os.path.join(root, file)
                filelist.append(f)

        print(f"NLMChem has {len(filelist)} files to parse")
